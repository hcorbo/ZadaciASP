...
public:
 ...
 void brisiOdAdoB(int a, int b){
     Cvor *p=p_pocetak, *q = p_pocetak, *prije_p = p_pocetak;
     int brojac_p=0, brojac_q=0;
     while(p->element != a){
         p = p->p_sljedeci;
         brojac_p++;
     }
     ...
     if(brojac_p!=0)
        while(prije_p->p_sljedeci != p){
         prije_p = prije_p->p_sljedeci;
     }
     Cvor *temp = p_trenutni;
     if(p>temp && temp<q) p_trenutni = p;
     p->p_sljedeci = q;
     brojEl-=abs(brojac_q-brojac_p-1);
 }

Cvor *pronadjiPrethodniCvor(Cvor *p){
    Cvor *temp = p_pocetak;
    if(temp == p) return nullptr;
    while(temp->p_sljedeci != p){
        temp = temp->p_sljedeci;
    }
    return temp;
}

int Suma(int sum, Cvor *p){
    if(p == p_kraj){
        std::cout << sum << std::endl;
        return sum;
    }
    sum += p->p_sljedeci->element;
    Suma(sum, p->p_sljedeci);
}
void brisiUnazadOdSumePrethodnih(){
    if(brojEl == 1 || brojEl == 0) return;
    Cvor *p = pronadjiPrethodniCvor(p_kraj);
    Cvor *q = p;
    std::cout << "tu" << std::endl;
    while(p != nullptr){
        q = p;
        if(Suma(0, p) > p->element){
            Cvor * temp = pronadjiPrethodniCvor(p);
            if(p_trenutni == p) p_trenutni = p->p_sljedeci;
            if(temp == nullptr){
                p_pocetak = p->p_sljedeci;
                q=p->p_sljedeci;
                delete p;
                brojEl--;
            } else {
                temp->p_sljedeci = p->p_sljedeci;
                q=p->p_sljedeci;
                delete p;
                brojEl--;
            }
        }
        p = pronadjiPrethodniCvor(q);
    }
    
}
 void Obrni1(int n, int m){
     if(n<0  ||n>m || n>brojEl || m>brojEl) throw std::logic_error("Nedozvoljeni indeksi");
     ...
     brojac = 0;
     Cvor *zadnji = p_pocetak;
     while(brojac<m){
         zadnji = zadnji->p_sljedeci;
         brojac++;
     }
     
     //dodati kada je p=p_pocetak
    if(n==0 && zadnji == p_kraj){
        Cvor *q = nullptr;
        while(p!=nullptr){
            std::cout<<"Dosao";
            Cvor *r = q;
    ...
    //ako je m kranji
    else if(zadnji == p_kraj){
        Cvor *prije_p = p_pocetak;
        while(prije_p->p_sljedeci!=p){
            prije_p = prije_p->p_sljedeci;
        }
        Cvor *prije_zadnjeg = p;
        while(prije_zadnjeg->p_sljedeci!=zadnji){
            prije_zadnjeg = prije_zadnjeg->p_sljedeci;
        }
    Cvor *q = p;
    Cvor *temp = p->p_sljedeci;
    while(n!=m-1){
     ...
     p->p_sljedeci = nullptr;
     prije_p->p_sljedeci = zadnji;
     zadnji->p_sljedeci = prije_zadnjeg;
     //postaviti kraj na p
     p_kraj = p;
    } else {
        //pokazivac na prije p
    Cvor *prije_p = p_pocetak;
        while(prije_p->p_sljedeci!=p){
            prije_p = prije_p->p_sljedeci;
     }
     //pokazivac na cvor prije zadnjeg
     Cvor *prije_zadnjeg = p;
        while(prije_zadnjeg->p_sljedeci!=zadnji){
            prije_zadnjeg = prije_zadnjeg->p_sljedeci;
     }  
     //obrcem sredisnje pokazivace
      Cvor *q = p;
      Cvor *temp = p->p_sljedeci;
     while(n!=m-1){
         Cvor *r = q;
     ...
     //postavjljam pokazivace kraja i pocetka
     p->p_sljedeci = zadnji->p_sljedeci;
     prije_p->p_sljedeci = zadnji;
     zadnji->p_sljedeci = prije_zadnjeg;
    }
 }
 ...
 void Obrni(int n, int m){
     ...
     Cvor *n_cvor = p_pocetak, *m_cvor = p_pocetak, *prije_n = p_pocetak;
     int brojac = 0;
     while (brojac < n){
     ...
     Cvor *poslije_m = m_cvor -> p_sljedeci;
     //obrtanje sredisnjih cvorova
     Cvor *r = n_cvor;
     Cvor *q = nullptr;
     while(q != m_cvor){
         Cvor *p = q;
     ...
     if(n == 0 && m_cvor==p_kraj){
         n_cvor->p_sljedeci = nullptr;
         p_pocetak = m_cvor;
         p_kraj = n_cvor;
     }
     else if(n == 0){
         n_cvor->p_sljedeci = poslije_m;
         p_pocetak = m_cvor;
     } else {
         n_cvor->p_sljedeci = poslije_m;
         if(m_cvor==p_kraj) p_kraj = n_cvor;
        prije_n->p_sljedeci = m_cvor;
     }
 }
 
 
 void Premjesti(int n, int m, int k){
     if(k>=n && k<=m) throw std::logic_error("Nije u dozvoljenom intervalu");
     
     Cvor *prije_n = p_pocetak, *n_cvor = p_pocetak, *m_cvor = p_pocetak, *prije_k = p_pocetak, *k_cvor = p_pocetak;
     int brojac=0;
     while(brojac<n-1){
         prije_n = prije_n->p_sljedeci;
         brojac++;
     ...
     brojac=0;
     while(brojac<k){
         k_cvor = k_cvor->p_sljedeci;
         brojac++;
     }
     
     if(k==0){
         prije_n->p_sljedeci = m_cvor->p_sljedeci;
         m_cvor->p_sljedeci = k_cvor;
         p_pocetak = n_cvor;
         if(m_cvor==p_kraj) p_kraj = prije_n;
     } else {
         prije_n->p_sljedeci = m_cvor->p_sljedeci;
         m_cvor->p_sljedeci = k_cvor;
         prije_k->p_sljedeci = n_cvor;
         if(m_cvor==p_kraj) p_kraj = prije_n;
     }
 }
 
 
 void PremjestiUzObrtanje(int n, int m, int k){
     if(k>=n && k<=m) throw std::logic_error("Nije u dozvoljenom intervalu");
...
//          n_cvor->p_sljedeci = k_cvor;
//          prije_k->p_sljedeci = m_cvor;
//          if(n_cvor==p_kraj) p_kraj = prije_n;
//      }
//  }
// };
...
int main()
{
    ...
    jed_lista.brisiUnazadOdSumePrethodnih();
    jed_lista.pocetak();
